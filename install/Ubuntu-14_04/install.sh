...
# should be 10.1.1 , no branch on github yet though
Sentora_VERSION=master
...
if dpkg -s php5 apache2 mysql bind postfix dovecot; then
...
if [ "$OS" = "Ubuntu" ] && [ "$VER" = "14.04" ]; then
  ...
  echo "Sorry, this installer only supports the installation of Sentora on Ubuntu 14.04."
  exit 1;
fi
...
# TODO = NOT-FOUND, deprecated? : php5-suhosin , db4.7-util
